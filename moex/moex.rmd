...
```{r, message=F, warning=F}
make_groups_new <- function(df_dailyTS, vVariables, nBoot = 8, vYears) {
  ...
  for (boot in 1:nBoot) {
    for (vYear in vYears) {
      vRowName <- paste0(boot,"_",vYear)
      vRowNames[length(vRowNames)+1] <- vRowName
      
      l_indexes[[vRowName]] <- get_index(df = df_dailyTS[which(df_dailyTS$year == vYear),])
    }
  }
...
for (vVariable in vVariables) {
  ...
  # create bootstrap df
  l_dailyTS <- list()
  for (vRowName in vRowNames) {
    l_dailyTS[[vRowName]] <- df_dailyTS[l_indexes[[vRowName]],]
  }
...
df_groups_new <- make_groups_new(df_dailyTS = df_dailyTS, vVariables = c("probTS"), nBoot = 1, vYears = vYears)
```



```{r, message=F, warning=F}
vYears <- unique(df_dailyTS$year)[sample(1:11,5)]
...
df_groups <- df_groups %>%
  ...
  mutate(profitSE = ProfitSD/sqrt(n),
         t0 = if_else(ProfitSD == 0, NaN ,(0 - profitMean/profitSE)),
...
```{r}
Rprof()
df_daily[,"prob0"] <- apply(df_daily,1,FUN = function(x) get_prob0m(x,
                                                                        vVariables,
                                                                        m_groupsW,
                                                                        m_variableValue,
                                                                        m_prob0,
