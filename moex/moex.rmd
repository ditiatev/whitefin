...
df_groups <- df_groups %>%
  filter(!is.nan(prob0)) %>%
  group_by(group,variableName) %>%
  mutate(grProb0 = mean(prob0)) %>%
  group_by(group,variableName) %>%
  mutate(sd_grProb0 = sd(grProb0),
         n_gr = length(unique(year)),
         se_grProb0 = sd_grProb0/sqrt(n_gr))

df_groups <- df_groups %>%
  group_by(variableName, group, se_grProb0) %>%
  ...
  mutate(variableFactor = variableFactor[1],
         seProb0 = if_else(seProb0 < 0.01, 0.01, seProb0),
         weight = 1/((6*seProb0)^2),
         se_grProb0 = if_else(se_grProb0 < 0.01, 0.01, se_grProb0),
         gr_weight = 30/((6*se_grProb0)))# %>%
  #mutate(weight = weight + gr_weight)
...
vYears <- unique(df_daily$year)[1:5]
...
df_profit <- df_profit %>%
  mutate(diff = (profitProb0-profit),
         forecast = if_else(year > 2014, T, F))
df_profit
```
