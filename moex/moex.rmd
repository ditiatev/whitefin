...
m_RowDaily <- as.matrix(as.numeric(df_dailyTS[1368,vVariables]))
m_RowDaily <- matrix(rep(m_RowDaily,each=vNRow),nrow=vNRow)

# create abs diff
m_min <- abs(m_groupsW[,paste0("variableValue.",vVariables)] - m_RowDaily)

# find index of abs diff
minindex <- apply(m_min, 2, which.min)
...
```{r}
which(m_min == min(m_min, na.rm = T), arr.ind=TRUE)
which(m_min == minindex, arr.ind=TRUE)
```
...
```{r}
get_prob0 <- function(vRow,vVariables,df_gr) {
  ...
  for (vVariable in vVariables) {
    ...
    df_GrStrVar <- df_gr[which(df_gr$variableName == vVariable),]
    ...
    df_prob0 <- dplyr::bind_rows(list(df_prob0, data.frame(variableName = vVariable,
                                                           ...
                                                           weight  = df_GrStrVar[index,"weight"])))
    }
  }
  
  vTotalWeight <- sum(df_prob0$weight, na.rm = T)
  
  df_prob0$prob0w <- df_prob0$weight/vTotalWeight*df_prob0$prob0
  vTotalProb <- sum(df_prob0$prob0w, na.rm = T)
