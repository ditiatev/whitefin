...
df_all <- df_strategies %>%
  ...
  mutate(profit_cum = if_else(is.na(profit),0,profit),
         ...
         profit_cum_l2 = if_else(lev2 >= vote_level, profit_cum ,0)) %>%
  mutate(profit_cum = cumsum(profit_cum),
         lev2_cum = cumsum(lev2),
         profit_cum_l2 = cumsum(profit_cum_l2),
...
df_test <- df_strategies %>%
  ...
  geom_point(aes(x = date, y = profit_diff), color = 'green', alpha = 0.1, data = df_test) +
...
df_strategies_m <- df_strategies %>%
  filter(#date < start_date,
    date < ymd('20200401')) %>%
  mutate(npv_up0 = if_else(vote >= vote_level, vote, 0),
         profit_up0 = if_else(vote >= vote_level, profit, 0)) %>%
  group_by(date) %>%
  mutate(n_up0 = n(),
         #npv_up0 = if_else(n_up0 >= 3, npv_up0, 0),
  ...
  #mutate(pers_sd = sd(pers, na.rm = T)) %>%
  #mutate(pers = if_else(pers > .35, .35, pers)) %>%
  mutate(money = 0,
         money_m = 0) %>%
  arrange(date)
