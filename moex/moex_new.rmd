...
```{r}
df_profit_total <- get_WorkSet(vStrategies[5])
...
vColNames <- c('value_IMOEX','value_MOEX10','value_MOEXFN','value_RTSI',
               ...
               "diffHL_RTSI","diffOC_RTSI","diffOL_RTSI","diffHO_RTSI",vColNames_level)
...
```{r}
df_vote_train_summary_parallel %>%
  filter(prob0_up > -3) %>%
        group_by(n) %>%
        summarise(n(),
                  mean(prob0_up))

l_str_save$l_lev1$test %>%
...
for (vStrategy in vStrategies[5]) {
  # data
...
l_str_save <- readRDS(paste0("./data/vote/",'base_SL_14_18_RTS_Int005.rds'))
debug(count_lev3)
...
for(vFile in vFiles) {
  ...
  l_strategies[vFile] <- list(df_lev3)

# df_lev2_train <- l_str_save$l_lev2$train[['20']]
# df_lev2_test <- l_str_save$l_lev2$test[which(l_str_save$l_lev2$test$MA == '20'), c("date","profit_pred")]
# df_lev2_20 <- rbind(df_lev2_train,df_lev2_test)
# colnames(df_lev2_20) <- c('date', 'pred_20')
# 
# df_lev2_train <- l_str_save$l_lev2$train[['25']]
# df_lev2_test <- l_str_save$l_lev2$test[which(l_str_save$l_lev2$test$MA == '25'), c("date","profit_pred")]
# df_lev2_25 <- rbind(df_lev2_train,df_lev2_test)
# colnames(df_lev2_25) <- c('date', 'pred_25')
# 
# df_lev2_train <- l_str_save$l_lev2$train[['30']]
# df_lev2_test <- l_str_save$l_lev2$test[which(l_str_save$l_lev2$test$MA == '30'), c("date","profit_pred")]
# df_lev2_30 <- rbind(df_lev2_train,df_lev2_test)
# colnames(df_lev2_30) <- c('date', 'pred_30')
# 
# df_lev2_train <- l_str_save$l_lev2$train[['35']]
# df_lev2_test <- l_str_save$l_lev2$test[which(l_str_save$l_lev2$test$MA == '35'), c("date","profit_pred")]
# df_lev2_35 <- rbind(df_lev2_train,df_lev2_test)
# colnames(df_lev2_35) <- c('date', 'pred_35')
# 
# df_lev2 <- merge(l_str_save$df_profit[,c("date","profit")], df_lev2_20, all.x = T, by = 'date')
# df_lev2 <- merge(df_lev2, df_lev2_25, all.x = T, by = 'date')
# df_lev2 <- merge(df_lev2, df_lev2_30, all.x = T, by = 'date')
# df_lev2 <- merge(df_lev2, df_lev2_35, all.x = T, by = 'date')
# 
# df <- df_lev2
# df <- na.omit(df)
# 
#   index_tain <- df$date < (l_str_save$topVars_date)
#   index_predict <- which(df$date > l_str_save$topVars_date)
#     
#   l <- create_X_y(df = df,
#                   train_index = index_tain,
#                   test_index = index_predict,
#                   vVars = c('pred_20','pred_25','pred_30','pred_35'))
#   
#   knn_test  <- knn.reg(train=l$X_train, y=l$y_train, test=l$X_test, k=50)
#   df[index_predict,'profit_pred'] <- knn_test[['pred']]
#   
#   knn_train  <- knn.reg(train=l$X_train, y=l$y_train, test=l$X_train, k=50)
#   df[index_tain,'profit_pred'] <- knn_train[['pred']]
# 
# df_lev2$profit_pred = apply(df_lev2[,c('pred_20','pred_25','pred_30','pred_35')], 1, mean, na.rm = T)
# 
# df_lev2$strategy <- vFile
# l_strategies[vFile] <- list(df_lev2)
# 
# print(
# ggplot(data = df_lev2) +
#   geom_point(aes(x = profit, y = profit_pred))
# )

}
df_strategies <- do.call(rbind,l_strategies)
```


```{r, warning=FALSE, message=FALSE}
l_best_test_vote_level <- list()
for (vote_level in 0:12) {
df_best_test_vote_level <- df_strategies %>%
 filter(date < ymd('20200401'),
        #date < start_date
        ) %>%
...
l_best_test_vote_level[vote_level] <- list(df_best_test_vote_level)
}
df_best_test_vote_level <- do.call(rbind, l_best_test_vote_level)
df_best_test_vote_level <- df_best_test_vote_level %>%
  group_by(strategy) %>%
  mutate(max_pr_diff = max(pr_diff)) %>%
  arrange(strategy, desc(pr_diff)) %>%
  filter(pr_diff == max_pr_diff) %>%
  mutate(best_vote_lev = vote_level) %>%
  select(strategy,best_vote_lev)

df_strategies <- merge(df_strategies,df_best_test_vote_level, all.x = T, by = 'strategy')
```
...
df_strategies %>%
 filter(date < ymd('20200401'),
  ...
  mutate(profit_ma = if_else(vote >= best_vote_lev, profit, 0),
         n_ma = if_else(vote >= best_vote_lev, 1, 0)) %>%
  group_by(type,
           test = date > start_date,
           #year(date)
           ) %>%
...
tt_test <- length(which(df_test$lev2 >= vote_level))/nrow(df_test)*100
  ...
  ggtitle(paste0(vFile,"  Vote level: ",vote_level)) + 
    ...
    annotate(geom="text", x=ymd('20110601'), y=90, 
      ...
      saveGGplot(sGGplot,branch,paste0(vFile,'_vl_',vote_level))
  }
}
opendir(branch)
```
