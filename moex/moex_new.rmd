...
head(df_profit_total[df_profit_total$date > last_work_date,],1)
...
if (is.null(l_str_save['df_profit'][[1]])) {
  ...
  l_str_save['work_date'] <- list(start_date)
} else {
  # date update
  last_work_date <- max(df_profit$date)
  l_str_save['last_work_date'] <- list(last_work_date)
  
  # code to get df_profit for next date
  df_profit_next <- head(df_profit_total[df_profit_total$date > last_work_date,],1)
  df_profit <- rbind(l_str_save$df_profit,df_profit_next)
  l_str_save['df_profit'] <- list(df_profit)
...
df_profitPrAll <- do.call(cbind,list(l_str_save$df_profit[,c("date","profit")],
...
if (!is.null(l_str_save['l_topVars'][[1]])) {
  df_unique_variabels <- get_df_unique_variabels(vVariables[1:50])
...
l_lev1_test <- list()
  ...
  df_vote_test_summary_parallel <- get_vote_summary_parallel(df_profitPr = df_profitPr,
                                                             df_topVars = df_topVars,
                                                             vVoteIndex = vVoteIndex,
                                                             core = 3,
                                                             days_lag = days_lag,
                                                             paralleling = FALSE)
  l_lev1_test[topVars_name] <- list(df_vote_test_summary_parallel)
  }
l_lev1['test'] <- list(l_lev1_test)
  
l_str_save['l_lev1'] <- list(l_lev1)
  
vVoteIndex <- which(df_profitPr$date == (l_str_save$work_date[1]-71))
vVoteIndex
df_vote_test_summary_parallel <- get_vote_summary_parallel(df_profitPr = df_profitPr,
                                                           df_topVars = df_topVars,
                                                           vVoteIndex = vVoteIndex,
